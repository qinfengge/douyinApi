<template>
  ...
  >
    <swiper-item v-for="(item, index) in state.displaySwiperList" :key="index">
      <view v-if="item.url" class="swiper-item" @click="handleClick">
        <video
        ...
        <image
          v-if="item.thumbnail && state.displayIndex != index"
          :src="item.thumbnail"
	  ...
	  <swiper v-if="item.images" :vertical="false" :autoplay="autoplay" :indicator-dots="true" class="m-tiktok-images-swiper" >
	  	<swiper-item v-for="(pic, index) in item.images" :key="index" >
	  			<view >
	  				<image :src="pic" mode="aspectFit" class="m-tiktok-images-image"></image>
...
function swiperChange(event: any) {
  ...
  // 如果两者的差为-2或者1则是向前滑动
    ...
    state.displayIndex - current == 1
	  ...
	  console.log("向前滚动" + state.originIndex)
